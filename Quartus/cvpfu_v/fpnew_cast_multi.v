module fpnew_cast_multi_2E827_67072 (
	...
	function automatic [31:0] fpnew_pkg_int_width;
		...
		case (ifmt)
			...
			default: begin
				$display("Fatal [%0t] C:\Users\jtori\OneDrive\Escritorio\cvfpu\fpnew_pkg.sv:96:9 - fpnew_cast_multi.fpnew_pkg_int_width.<unnamed_block>\n msg: ", $time, "Invalid INT format supplied");
				$finish(1);
	...
	generate
		for (_gv_i_3 = 0; _gv_i_3 < NUM_INP_REGS; _gv_i_3 = _gv_i_3 + 1) begin : gen_input_pipeline
	...
	genvar _gv_i_4;
	generate
		for (_gv_i_4 = 0; _gv_i_4 < NUM_MID_REGS; _gv_i_4 = _gv_i_4 + 1) begin : gen_inside_pipeline
	...
	always @(*) begin : cast_value
		...
		if (dst_is_int_q) begin
			...
			if ((input_exp_q >= $signed((fpnew_pkg_int_width(int_fmt_q2) - 1) + op_mod_q2)) && !((!op_mod_q2 && input_sign_q) && (input_exp_q == $signed(fpnew_pkg_int_width(int_fmt_q2) - 1)))) begin
				denorm_shamt = 1'sb0;
	...
	genvar _gv_fmt_6;
	generate
		for (_gv_fmt_6 = 0; _gv_fmt_6 < sv2v_cast_32_signed(NUM_FORMATS); _gv_fmt_6 = _gv_fmt_6 + 1) begin : gen_res_assemble
	...
	genvar _gv_ifmt_4;
	generate
		for (_gv_ifmt_4 = 0; _gv_ifmt_4 < sv2v_cast_32_signed(NUM_INT_FORMATS); _gv_ifmt_4 = _gv_ifmt_4 + 1) begin : gen_int_res_zero_ext
	...
	genvar _gv_fmt_7;
	generate
		for (_gv_fmt_7 = 0; _gv_fmt_7 < sv2v_cast_32_signed(NUM_FORMATS); _gv_fmt_7 = _gv_fmt_7 + 1) begin : gen_sign_inject
			...
			if (FpFmtConfig[fmt]) begin : active_format
				always @(*) begin : post_process
					if (_sv2v_0)
						;
					fmt_uf_after_round[fmt] = (rounded_abs[(EXP_BITS + MAN_BITS) - 1:MAN_BITS] == {(((EXP_BITS + MAN_BITS) - 1) >= MAN_BITS ? (((EXP_BITS + MAN_BITS) - 1) - MAN_BITS) + 1 : (MAN_BITS - ((EXP_BITS + MAN_BITS) - 1)) + 1) * 1 {1'sb0}}) || (((pre_round_abs[(EXP_BITS + MAN_BITS) - 1:MAN_BITS] == {(((EXP_BITS + MAN_BITS) - 1) >= MAN_BITS ? (((EXP_BITS + MAN_BITS) - 1) - MAN_BITS) + 1 : (MAN_BITS - ((EXP_BITS + MAN_BITS) - 1)) + 1) * 1 {1'sb0}}) && (rounded_abs[(EXP_BITS + MAN_BITS) - 1:MAN_BITS] == 1)) && ((round_sticky_bits != 2'b11) || (!destination_mant[NUM_FP_STICKY - 1] && ((rnd_mode_q == 3'b000) || (rnd_mode_q == 3'b100)))));
	...
	genvar _gv_ifmt_5;
	generate
		for (_gv_ifmt_5 = 0; _gv_ifmt_5 < sv2v_cast_32_signed(NUM_INT_FORMATS); _gv_ifmt_5 = _gv_ifmt_5 + 1) begin : gen_int_res_sign_ext
	...
	assign rounded_int_res_zero = (rounded_int_res == {WIDTH {1'sb0}}) && (pre_round_abs == {WIDTH {1'sb0}});
	genvar _gv_ifmt_6;
	generate
		for (_gv_ifmt_6 = 0; _gv_ifmt_6 < sv2v_cast_32_signed(NUM_INT_FORMATS); _gv_ifmt_6 = _gv_ifmt_6 + 1) begin : gen_int_overflow
			...
			if (IntFmtConfig[ifmt]) begin : active_format
				always @(*) begin : detect_overflow
					...
					if ((!op_mod_q2 && rounded_sign) && (input_exp_q == $signed(INT_WIDTH - 1)))
	...
	genvar _gv_fmt_8;
	generate
		for (_gv_fmt_8 = 0; _gv_fmt_8 < sv2v_cast_32_signed(NUM_FORMATS); _gv_fmt_8 = _gv_fmt_8 + 1) begin : gen_special_results
	...
	genvar _gv_ifmt_7;
	generate
		for (_gv_ifmt_7 = 0; _gv_ifmt_7 < sv2v_cast_32_signed(NUM_INT_FORMATS); _gv_ifmt_7 = _gv_ifmt_7 + 1) begin : gen_special_results_int
	...
	genvar _gv_i_5;
	generate
		for (_gv_i_5 = 0; _gv_i_5 < NUM_OUT_REGS; _gv_i_5 = _gv_i_5 + 1) begin : gen_output_pipeline
